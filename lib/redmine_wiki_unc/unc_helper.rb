module RedmineWikiUnc
  class UncHelper
    ...
    def get_tag(args)
      ...
      html += "<button class='path_unc icon-only icon icon-copy'" \
          " data-clipboard-text='#{unc}'" \
          " title='#{I18n.t(:message_copy_path)}'" \
          " onclick='WikiUnc.fn.copyTextToClipboard(this, arguments[0])'" \
          " onmouseover='$(this).attr({title: \"#{I18n.t(:message_copy_path)}\"})'>" \
          "</button>"
      html += "</span>"
